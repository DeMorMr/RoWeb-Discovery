...
class RoWebApp {
    ...
    async loadThumbnails(placesToShow, size = 64) { //128
    ...
    async getBatchThumbnailUrls(placeIds, size = 64) { //128
        const uncachedIds = [];
        const result = {};
    ...
    async loadCoolThumbnails(places) {
        for (let i = 0; i < places.length; i += this.BATCH_SIZE) {
            ...
            try {
                const thumbnails = await this.getBatchThumbnailUrls(placeIds, 64); //128
                validPlaces.forEach(place => {
                    const img = document.querySelector(`.UserPlace img[data-place-id="${place.id}"]`);
                    if (img) {
                        img.src = thumbnails[place.id] || 'data/main/NewFrontPageGuy.png';
...
function sfx(name) { return app.sfx(name); }
